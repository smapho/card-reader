...
## 2.5. Vercel と Supabase の連携

1. Vercel でこのリポジトリをインポートします。
2. Vercel の Project Settings → Environment Variables に以下を登録します。
   - `GOOGLE_GENERATIVE_AI_API_KEY`
   - `NEXT_PUBLIC_SUPABASE_URL`
   - `NEXT_PUBLIC_SUPABASE_ANON_KEY`
   - `SUPABASE_SERVICE_ROLE_KEY`
3. Vercel のビルドコマンドは `npm run build`、出力ディレクトリは `.` のままで大丈夫です。
4. Supabase 側で `supabase/migrations/001_receipts.sql` を実行してテーブルとストレージを用意します。

> `.vercel/project.json` はこのリポジトリがすでに Vercel プロジェクトとリンクされていることを示します。
